...
run_syntax() {
  ...
  for f in main.sh sync.sh setup-sec-lab.sh lab-manager.sh termux-ethical-lab-install.sh core-command.sh tests/test_core_command.sh; do
...
case "${1:-all}" in
  all) run_syntax; run_unit; run_security; echo 'test-manager: PASS' ;;
